...
from controllers.SecretDitoUserConversationController import (
    SecretDitoUserConversationController,
)
...
if __name__ == "__main__":
    ...
    user_controllers = SecretDitoUserControllers(
        container.resolve(ISecretDitoRepo), container.resolve(FlagServiceProtocol)
    )
    conversation_handler = SecretDitoUserConversationController(
        container.resolve(ISecretDitoRepo)
    )

    print("Registering handlers...")
    app.add_handler(
        ConversationHandler(
            entry_points=[CommandHandler("registro", conversation_handler.entry_point)],
            states={
                SecretDitoUserConversationController.ConversationState.SET_NAME: [
                    MessageHandler(None, conversation_handler.set_name_state)
                ],
            },
            fallbacks=[],
        )
    ...
    app.add_handler(
        CommandHandler("secret_friend", user_controllers.get_secret_friend_handler)
    )
    app.add_handler(
        CommandHandler(
            "secret_wish_list", user_controllers.get_secret_friend_wish_list_handler
        )
    )
    app.add_handler(
        MessageReactionHandler(
            user_controllers.delete_from_wish_list_by_reaction_handler
        )
    )
